...
pub(crate) static CODEX_BACKEND: CodexBackend = CodexBackend;
...
fn import_from(config: &mut MultiAppConfig) -> Result<usize, AppError> {
    ...
    let text = crate::config::codex::read_and_validate_codex_config_text()?;
...
fn sync_enabled(config: &MultiAppConfig) -> Result<(), AppError> {
    ...
    let path = crate::config::codex::get_codex_config_path();
...
fn sync_single(id: &str, server_spec: &Value) -> Result<(), AppError> {
    ...
    // 读取现有的 config.toml
    let config_path = crate::config::codex::get_codex_config_path();
