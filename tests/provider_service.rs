...
"#
    ...
    let servers = initial_config.mcp.servers.get_or_insert_with(HashMap::new);
...
#[test]
fn provider_service_switch_codex_without_auth_writes_config_only() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    ProviderService::switch(&state, AppType::Codex, "invalid")
        .expect("switching should succeed without auth");

    let auth_path = home.join(".codex").join("auth.json");
    assert!(
        !auth_path.exists(),
        "auth.json should remain absent when provider has no auth"
    );
}
